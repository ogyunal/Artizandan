...
class CartController extends controller
{
	public function addon_add_to_cart(Request $request)
	{
		...
		Cart::instance('cart_'.$request->store_slug)->add($single_product->id, $single_product->title, $request->qty_value, $single_product->price->price, 550,['type'=>'Regular']);
	...
	public function add_to_cart(Request $request)
	{
		...
		Cart::instance('cart_'.$request->store_slug)->add($product->id, $product->title, 1, $product->price->price, 550,['type'=>'Regular']);
