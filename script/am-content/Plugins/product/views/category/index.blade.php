	...
	@endif
		...
		<div class="card">
			<div class="card-body">
				@php
				if (!empty($req)) {
					$categeories=\App\Category::where('type',$type)->where('name','LIKE','%'.$req.'%' )->with('user')->latest()->paginate(12);	
				}
				else{
					$categeories=\App\Category::where('type',$type)->with('user')->latest()->paginate(12);
				}
				@endphp
